...
class NeuralNetwork(object):
    def __init__(self, input_nodes, hidden_nodes, output_nodes, learning_rate):
        ...
        self.weights_hidden_to_output = np.random.normal(0.0, self.hidden_nodes**-0.5, 
                                       (self.hidden_nodes, self.output_nodes)) # output nodes instead of hidden nodes for mean
    ...
    def backpropagation(self, final_outputs, hidden_outputs, X, y, delta_weights_i_h, delta_weights_h_o):
        ...
        hidden_error_term = hidden_error * hidden_outputs*(1-hidden_outputs) # f'(x) sigmoid * (1-sigmoid)
        
        # Weight step (input to hidden)
        delta_weights_i_h += X[:,None].dot(hidden_error_term)
        # Weight step (hidden to output)
        delta_weights_h_o += hidden_outputs[:,None].dot(output_error_term)
...
iterations = 6000
learning_rate = .5
hidden_nodes = 25 # waren ursprünglich 2 
output_nodes = 1